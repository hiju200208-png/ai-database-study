...
| 확인 항목 | 실제 결과 | 의미 |
| --- | --- | --- |
| current_database() | ai_database_book | 현재 사용하는 데이터베이스 이름 |
| current_user | postgres | 사용자 계정 |
| current_schema() | public | 현재 사용 스키마 |
| search_path | "$user", public | 스키마 이름을 생략했을 때 PostgreSQL이 테이블 등을 찾는 스키마 검색 순서 |
| transaction_read_only | off | 현재 연결이 읽기 전용이 아니어서 쓰기 작업이 가능한 상태 |
...
테이블 생성 성공 여부: yes
실제 행 수: 6
DBeaver에서 확인한 위치: ai_database_book
```
...
| 열 | 타입 | NULL 가능? | 역할 |
| --- | --- | --- | --- |
| id | INTEGER | 불가능 | 학생 한 명을 구분하기 위한 고유 식별자(PK) |
| name | VARCHAR | 불가능 | 학생 이름 저장 |
| email | VARCHAR | 불가능 | 학생 이메일 주소 저장 |
| major | VARCHAR | 가능 | 학생의 전공 정보 저장 |
| grade | INTEGER | 가능 | 학생 학년 정보 저장 |
| created_at | TIMESTAMPTZ | 불가능 | 학생 정보가 등록된 날짜와 시간 저장 |

### `id`를 학번이나 학생 수로 해석하면 안 되는 이유

```text
자동 번호는 학생 수나 학번이 아니라 행을 구분하는 내부 식별자이기 때문이다.
```
...
| 1 | 전체 학생 | 6 | 6 | 일치 |  |
| 2 | 이름·이메일만 조회 | 6 | 6 | 일치 |  |
| 3 | 특정 전공 | 해당 전공의 학생 수 | 2 | 일치 |  |
| 4 | 특정 학년 이상 | 해당 학년의 학생 수 | 2 | 일치 |  |
| 5 | 두 전공 중 하나 | 해당 전공 학생 수 | 3 | 일치 |  |

...
```text
AI가 설명한 내용: 각 수강 기록을 무엇으로 고유하게 식별할 것인가?”를 결정해야 해.

본문에서 확인한 내용: 각 행을 고유하게 구분하는 PK가 필요하다

일치 / 부분 일치 / 수정 필요: 일치

내가 최종적으로 이해한 내용: 각 테이블은 행을 고유하게 식별할 수 있도록 PK(Primary Key)를 두는 것이 일반적이다.
```
...
![PostgreSQL 현재 위치 확인](./images/step07_ai_review.png)
---
...
```text
서비스 이름:[모아]
서비스 목적:[동아리의 회원, 모임, 출석, 회비, 활동 등의 기록을 관리하기 위한 서비스]
```
...
| --- | --- | --- | --- |
| members | 동아리 회원 한 명의 정보 | member_id | member_number |
| activities | 동아리 활동 한 건의 정보 | activity_id | activity_code |
| participations | 한 회원이 한 활동에 참여한 기록 | participation_id | 확인 필요 |

## 8-4. FK 후보

```text
1. participations.member_id → members.member_id 참조 후보
   이유: 어떤 회원의 활동 참여 기록인지 연결하기 위해서이다.

2. participations.activity_id → activities.activity_id 참조 후보
   이유: 어떤 활동에 대한 참여 기록인지 연결하기 위해서이다.
```

## 8-5. 자연어 관계 문장

```text
1. 한 회원은 여러 활동에 참여할 수 있다.
2. 여러 회원은 한 활동에 참여할 수 있다.
3. 활동 참여 기록 한 건은 한 명의 회원과 하나의 활동을 참조한다.
```

## 8-6. 아직 확정하지 않을 정책

```text
Q1.동아리 회원이 탈퇴한 경우 기존 활동 참여 기록을 삭제할 것인가 보존할 것인가?
Q2.한 회원이 같은 활동에 중복으로 참여 등록할 수 있는가?
Q3.활동 참여를 취소한 경우 참여 기록을 삭제할 것인가 취소 상태로 남길 것인가?
```
...
```text
1. 활동을 만든 회원이나 담당 회원을 기록해야 하나요? 그렇다면 activities와 members 사이에 추가 관계가 필요한가요?
2.members, activities, participations는 데이터베이스가 아니라 테이블 후보라는 점을 구분하고 있나요?
3. participations의 “한 회원이 한 활동에 참여한 기록”에서 신청, 실제 참석, 취소 중 어느 시점의 기록을 의미하나요?
```

## 9-3. AI가 너무 빨리 결정한 내용 또는 내가 보류한 내용

```text
1.한 회원이 같은 활동에 여러 번 등록할 수 있다면 participations의 한 행 의미가 달라지지는 않나요?
2.현재 작성한 FK 설명에서는 PK와 FK의 기본 역할을 잘못 이해한 부분은 특별히 보이지 않아.
```
...
| 수정 전 | 수정 후 | 수정 이유 |
| --- | --- | --- |
| members 업무 식별자 후보: member_number | member_number → 확인 필요 | 실제 동아리에서 회원번호를 사용하고 고유하게 관리하는지 아직 확인되지 않았기 때문 |
| activities 업무 식별자 후보: activity_code | activity_code → 확인 필요 | 실제 업무에서 활동 코드를 사용하는지, 활동마다 고유한지 아직 확인되지 않았기 때문 |
| participations: 한 회원이 한 활동에 참여한 기록 | participations: 한 회원의 한 활동에 대한 참여 기록 | 신청·참여·취소 중 어떤 상태까지 포함하는지 아직 확정되지 않았기 때문에 |
...
```text
PostgreSQL은 ____데이터베이스 관리 시스템________ 이다.

DBeaver 또는 psql은 _______PostgreSQL에 접속하여 SQL을 실행하고 데이터를 확인하는 도구________ 이다.

데이터베이스와 스키마의 차이는 ___데이터베이스가 더 큰 단위이고 스키마는 그 안에서 테이블 등의 객체를 구분하고 관리하는 공간이라는 것_____ 이다.

테이블 한 행은 _____하나의 대상이나 하나의 업무 기록을 의미_______ 이다.

조회 결과가 원본 테이블과 다른 이유는 __SELECT, JOIN, WHERE 등의 SQL에 따라 필요한 데이터를 선택하거나 조합해서 보여주기 때문____ 이다.

내부 식별자와 업무 식별자의 차이는 ___내부 식별자는 DB 내부에서 데이터를 구분하기 위한 값이고 업무 식별자는 실제 업무에서 대상을 구분하기 위해 사용하는 값이라는 것____ 이다.

PK는 ______ 테이블의 각 행을 고유하게 식별하기 위한 키______ 이다.

FK는 _____다른 테이블의 행을 참조하여 테이블 사이의 관계를 연결하기 위한 키___ 이다.
```
